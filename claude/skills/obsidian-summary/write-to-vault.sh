...
# Usage: echo "<markdown>" | bash write-to-vault.sh <type> <project> <label-or-empty> <filename>
#
# Arguments:
#   $1 — note type: session | document | decision | investigation
#   $2 — project name (e.g. emerald-grove-pet-clinic)
#   $3 — label (only used for type=session; the "<label>" subfolder under
#        the project, e.g. "clinic_issue_1"). Pass "" for other types.
#   $4 — filename to write, e.g. "2026-03-04-session.md" (must not contain "/")
#
# Environment:
...
case "$TYPE" in
  session)      SUBFOLDER="AI Sessions" ;;
  document)     SUBFOLDER="Docs" ;;
  decision)     SUBFOLDER="Decisions" ;;
  investigation) SUBFOLDER="Investigations" ;;
  *)
    log_error "Invalid type '$TYPE'. Must be one of: session, document, decision, investigation."
    exit 1
    ;;
esac
...
if [[ "$TYPE" == "session" && -z "$LABEL" ]]; then
  log_error "Type 'session' requires a label."
  exit 1
fi
