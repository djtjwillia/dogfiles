...
name: synod-marsh
description: >
  Security reviewer. Use when the task involves auth, tokens, OIDC, OAuth, SSO,
  secrets, vault, credentials, encryption, CVEs, or supply chain concerns. Must be
  consulted before implementation when security is in scope. Holds veto on security.
